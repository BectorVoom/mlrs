...
- [ ] **FOUND-01**: A Cargo workspace splits compute kernels, backend/runtime selection, algorithms, and Python bindings into separate single-responsibility crates (`mlrs-core`, `mlrs-kernels`, `mlrs-backend`, `mlrs-algos`, `mlrs-py`)
- [ ] **FOUND-02**: Compute kernels are generic over float type (`f32`/`f64`) and over the CubeCL runtime, compiled once in a feature-free kernels crate
- [ ] **FOUND-03**: A backend is selected via Cargo features (`cuda`, `rocm`, `wgpu`, `cpu`); `cuda` compiles in this environment even though it cannot be run, and `wgpu`+`cpu` run as the correctness gate
...
- [ ] **FOUND-06**: Input data crosses into Rust as Apache Arrow buffers and feeds CubeCL device buffers zero-copy, with validation of offsets, null bitmaps, and alignment before any unsafe transmutation
- [ ] **FOUND-07**: An oracle test harness generates seeded random inputs, runs scikit-learn to produce reference outputs, and asserts results match within abs/rel error ≤ 1e-5
- [ ] **FOUND-08**: The oracle harness provides sign-flip (for SVD/PCA components) and label-permutation (for clustering) comparison helpers, plus a documented per-estimator f32 tolerance policy
...
- [ ] **PRIM-01**: A backend-portable GEMM / matrix-multiply primitive is available to estimators and validated against an oracle
- [ ] **PRIM-02**: Numerically-stable reduction primitives (sum/mean/min/max/argmin) work with a plane/subgroup path and a shared-memory fallback that both pass on wgpu
- [ ] **PRIM-03**: A pairwise-distance primitive (Euclidean/squared) with a `max(d², 0)` clamp serves KMeans, DBSCAN, and KNN

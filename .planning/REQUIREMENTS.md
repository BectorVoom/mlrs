...
- [ ] **BLDR-02**: The fit/unfit distinction is modeled as compile-time typestate (`T<Unfit>` → `T<Fitted>`); `predict` / `transform` / fitted-attr accessors exist only on `T<Fitted>`, preventing predict-before-fit at compile time (the hybrid Rust-surface design).
...
- [ ] **BLDR-04**: The PyO3 surface is unchanged — the Rust typestate collapses behind the existing `any_estimator!` `Unfit/F32/F64` enum, with a runtime `NotFittedError` analog at the Python boundary.

#!/usr/bin/env bash
# Run the Stacking{Regressor,Classifier} python suites against the extension
# currently installed in `crates/mlrs-py/python/mlrs/`, from THIS checkout.
#
...
exec "$VENV_PY" -m pytest \
    ...
    crates/mlrs-py/python/tests/test_oracle_stacking_classifier.py \

...
fn weighted_fit_equals_repeated_fit() {
    ...
    capability::log_oracle_dtype(capability::FloatKind::F64, backend, "default");
    if capability::skip_f64_with_log() {
        println!("multinomial_nb sample_weight f64 backend={backend}: SKIPPED (no f64 support)");
        return;
    }
    let (x, y, n, d, _c) = par_dataset();
    let w = int_weights(n);
    ...
    let client = runtime::active_client();
    let mut pool: BufferPool<ActiveRuntime> = BufferPool::new(client);
    let weighted = MultinomialNB::<f64>::builder()
        .build::<f64>()
        .expect("builds")
        .fit_from_host_slice(&mut pool, &x, &y, (n, d), Some(&w))
        .expect("weighted fit");
    let repeated = MultinomialNB::<f64>::builder()
        .build::<f64>()
        .expect("builds")
...
fn all_ones_weight_equals_unweighted() {
    ...
    capability::log_oracle_dtype(capability::FloatKind::F64, backend, "default");
    if capability::skip_f64_with_log() {
        println!("multinomial_nb ones-weight f64 backend={backend}: SKIPPED (no f64 support)");
        return;
    }
    let (x, y, n, d, _c) = par_dataset();
    ...
    let plain = MultinomialNB::<f64>::builder()
        .build::<f64>()
        .expect("builds")
        .fit_from_host_slice(&mut pool, &x, &y, (n, d), None)
...
fn fit_rejects_bad_sample_weight() {
    ...
    let client = runtime::active_client();
    let mut pool: BufferPool<ActiveRuntime> = BufferPool::new(client);
    let build = || MultinomialNB::<f64>::builder().build::<f64>().expect("builds");

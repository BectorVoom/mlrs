...
fn r2_score_constant_target_pins_sklearn_actual_value() {
    ...
    let got = r2_score::<f64>(&y_true_const, &y_pred_const, None).unwrap();
    assert_close(
        got,
...
fn r2_score_perfect_prediction_is_one() {
    ...
    let got = r2_score::<f64>(&y_perfect, &y_perfect, None).unwrap();
...
fn r2_score_matches_sklearn_oracle_f64() {
    ...
    let got = r2_score::<f64>(&y_true, &y_pred, None).unwrap();
...
fn r2_score_matches_sklearn_oracle_f32() {
    ...
    let got = r2_score::<f32>(&y_true, &y_pred, None).unwrap();
...
fn r2_score_weighted_matches_sklearn_oracle() {
    ...
    let got = r2_score::<f64>(&y_true, &y_pred, Some(&sw)).unwrap();
...
fn mean_squared_error_perfect_prediction_is_zero() {
    let case = load("metrics_reg_f64_seed42.npz");
    let y_perfect = f64_vec(&case, "y_perfect");
    let got = mean_squared_error::<f64>(&y_perfect, &y_perfect, None).unwrap();
...
fn mean_squared_error_matches_sklearn_oracle_f64() {
    ...
    let y_true = f64_vec(&case, "y_true");
    let y_pred = f64_vec(&case, "y_pred");
    let got = mean_squared_error::<f64>(&y_true, &y_pred, None).unwrap();
...
fn mean_squared_error_matches_sklearn_oracle_f32() {
    ...
    let y_true = f32_vec(&case, "y_true");
    let y_pred = f32_vec(&case, "y_pred");
    let got = mean_squared_error::<f32>(&y_true, &y_pred, None).unwrap();
...
fn mean_squared_error_weighted_matches_sklearn_oracle() {
    ...
    let y_pred = f64_vec(&case, "y_pred");
    let sw = f64_vec(&case, "sample_weight");
    let got = mean_squared_error::<f64>(&y_true, &y_pred, Some(&sw)).unwrap();
...
fn mean_absolute_error_perfect_prediction_is_zero() {
    let case = load("metrics_reg_f64_seed42.npz");
    let y_perfect = f64_vec(&case, "y_perfect");
    let got = mean_absolute_error::<f64>(&y_perfect, &y_perfect, None).unwrap();
...
fn mean_absolute_error_matches_sklearn_oracle_f64() {
    ...
    let y_true = f64_vec(&case, "y_true");
    let y_pred = f64_vec(&case, "y_pred");
    let got = mean_absolute_error::<f64>(&y_true, &y_pred, None).unwrap();
...
fn mean_absolute_error_matches_sklearn_oracle_f32() {
    ...
    let y_true = f32_vec(&case, "y_true");
    let y_pred = f32_vec(&case, "y_pred");
    let got = mean_absolute_error::<f32>(&y_true, &y_pred, None).unwrap();
...
fn mean_absolute_error_weighted_matches_sklearn_oracle() {
    ...
    let y_pred = f64_vec(&case, "y_pred");
    let sw = f64_vec(&case, "sample_weight");
    let got = mean_absolute_error::<f64>(&y_true, &y_pred, Some(&sw)).unwrap();
    assert_close(got, scalar(&case, "ref_mae_sw"), TOL, "mae weighted");
}

// ==================== code-review fix: sample_weight validation, no panic ====================
//
// r2_score/mean_squared_error/mean_absolute_error previously indexed
// `sample_weight[i]` with no length check. These regression tests lock in
// the fixed contract: a mismatched-length or invalid (negative/NaN)
// `sample_weight` returns a typed `Err`, never a panic.

#[test]
fn regression_metrics_bad_sample_weight_returns_err_not_panic() {
    let y_true = [1.0f64, 2.0, 3.0];
    let y_pred = [1.5f64, 2.5, 2.5];
    let too_short = [1.0f64];
    assert!(matches!(
        r2_score::<f64>(&y_true, &y_pred, Some(&too_short)),
        Err(MetricError::LengthMismatch)
    ));
    assert!(matches!(
        mean_squared_error::<f64>(&y_true, &y_pred, Some(&too_short)),
        Err(MetricError::LengthMismatch)
    ));
    assert!(matches!(
        mean_absolute_error::<f64>(&y_true, &y_pred, Some(&too_short)),
        Err(MetricError::LengthMismatch)
    ));

    let negative = [1.0f64, -1.0, 1.0];
    assert!(matches!(
        r2_score::<f64>(&y_true, &y_pred, Some(&negative)),

//! Plan 11-01 Wave-0 — ComplementNB (NB-04) oracle test SCAFFOLDS.
//!
...
//! per-variant `norm_true` case (the second L1 normalization of the complement
//! weights). ComplementNB decodes with `argmin` INTERNALLY (D-08) — same trait
//! surface, different decision rule. f64 cases carry the `skip_f64_with_log`
//! gate (D-07). Per AGENTS.md §2 tests live in `crates/mlrs-algos/tests/`.
...
/// HARD GATE (Wave 1): predict labels match sklearn EXACTLY, f32 (argmin decode).
#[test]
#[ignore = "Wave-1 (11-03): ComplementNB::fit is todo!() in Wave 0"]
fn exact_labels_f32() {
...
/// Per-variant (Wave 1): norm=true applies the second L1 normalization.
#[test]
#[ignore = "Wave-1 (11-03): ComplementNB::fit is todo!() in Wave 0"]
fn norm_true() {
    let case = load_npz(fixture("complement_nb_f64_seed42.npz")).expect("load complement_nb_f64");
...
fn build_rejects_bad_alpha() {
    // Wave 1: assert ComplementNB::<f64>::builder().alpha(-1.0).build::<f64>() is InvalidAlpha.
}

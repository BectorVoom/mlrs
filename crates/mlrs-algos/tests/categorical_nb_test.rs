//! Plan 11-01 Wave-0 — CategoricalNB (NB-05) oracle test SCAFFOLDS.
//!
//! `#[ignore]`-marked Wave-1 (11-04) scaffolds: load the committed fixture and
//! assert its SHAPE only (the `fit` body is `todo!()` in Wave 0). Adds the
//! per-variant `min_categories` case (the `MinCategories::{Uniform,PerFeature}`
//! padding) and `fit_rejects_bad_input` (negative / non-integer categorical
//! input → `AlgoError::InvalidCategoricalInput`). f64 cases carry the
//! `skip_f64_with_log` gate (D-07). Per AGENTS.md §2 tests live in
...
#[test]
#[ignore = "Wave-1 (11-04): CategoricalNB::fit is todo!() in Wave 0"]
fn exact_labels_f32() {
...
/// Per-variant (Wave 1): MinCategories::{Uniform,PerFeature} padding.
#[test]
#[ignore = "Wave-1 (11-04): CategoricalNB::fit is todo!() in Wave 0"]
fn min_categories() {
    let case = load_npz(fixture("categorical_nb_f64_seed42.npz")).expect("load categorical_nb_f64");
...
/// Per-variant (Wave 1): negative / non-integer categorical input →
/// AlgoError::InvalidCategoricalInput.
#[test]
#[ignore = "Wave-1 (11-04): CategoricalNB::fit is todo!() in Wave 0"]
fn fit_rejects_bad_input() {
    // Wave 1: fit on a negative / non-integer X → AlgoError::InvalidCategoricalInput.
}

/// build()-rejection (Wave 1): alpha < 0 → BuildError::InvalidAlpha.
#[test]
#[ignore = "Wave-1 (11-04): build-validation assertion lands with the fit body"]
fn build_rejects_bad_alpha() {
    // Wave 1: assert CategoricalNB::<f64>::builder().alpha(-1.0).build::<f64>() is InvalidAlpha.
}

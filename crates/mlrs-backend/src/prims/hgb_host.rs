...
///
/// The fit crosses several barriers per level — thousands per fit — so the
/// fast path has to be cheap: a waiter first spins (the phases it separates
/// are tens of microseconds) and then yields.
///
...
struct Barrier {
    ...
    /// fit re-raises on the calling thread.
    poisoned: AtomicBool,
...
/// Yields before a waiter parks (see [`Barrier::wait`]).
///
/// Deliberately LARGE. Measured: parking early is a big net loss — condvar
/// wake latency is tens of microseconds and the barriers fire thousands of
/// times per fit, so an eager third stage erased the pool's speedup entirely
/// (a 2-worker fit came out slower than a 1-worker one). Yielding is the
/// stage that actually carries an oversubscribed pool; parking exists only so
/// a pathologically starved worker cannot spin a core forever.
const YIELD_ITERS: u32 = 100_000;

/// A `&mut [T]` shared across the worker pool, whose disjointness is enforced
/// by the fit's task decomposition rather than by the borrow checker.
///
/// ## Safety contract (upheld by every use in this module)
...
// SAFETY: `Shared` is only ever handed to workers inside the `thread::scope`
// that owns the backing allocation, and access is disciplined by the contract
